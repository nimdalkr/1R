...
내 물건·미배치 짐 관리, 8개 배치안과 같은 시점 비교, 구조 잠금, 현관/침대 카메라, 스탠바이미 시청선, 파라메트릭 3D, 시작 화면과 빠른 실행을 연결했습니다. 최신 화면은 `assets/studio.js`에서 시작합니다. [기능·한계·검수 방법](docs/STUDIO.md)을 확인하세요.

작업은 **현재 브라우저에만 저장**되며 이전 JSON 파일을 불러올 수 있습니다. 실제 API 키 없이도 방 편집·저장·비교는 동작합니다. 상품 추천은 실제 규격 검수가 끝난 옵션만 공개하며 현재 승인 상품은 0개입니다.
