import {test} from 'node:test';import assert from 'node:assert/strict';import {spawn,execFileSync} from 'node:child_process';import {fileURLToPath} from 'node:url';
const root=fileURLToPath(new URL('../',import.meta.url));
test('real local HTTP server: public assets, API routing and private-file protection',async(t)=>{
 // Always test a fresh build, never a stale dist directory from an earlier editor version.
 ...
 for(const [path,status] of [['/',200],['/assets/studio.js',200],['/assets/plan.js',200],['/assets/mesh.js',200],['/assets/planner.js',404],['/assets/commerce.js',200],['/shared/catalog.mjs',200],['/catalog.json',200],['/build-info.json',200],['/admin/',200],['/docs/product-policy.html',200],['/.env.local',404],['/.env.example',404],['/server/coupang.mjs',404],['/data/catalog.json',404],['/tests/fixtures/catalog.mjs',404]]) {
  const res=await fetch(base+path);assert.equal(res.status,status,path);
 }
 const info=await (await fetch(base+'/build-info.json')).json();assert.equal(info.version,'1.4.0');assert.match(info.sourceDigest,/^[a-f0-9]{64}$/);
