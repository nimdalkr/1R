import {newDocument,makeObject,normalizeDocument,footprint} from './planner-model.mjs';
import {placementWarnings} from './catalog.mjs';
/* Starter examples, not measured building plans. Saved user layouts are never rewritten. */
...
export function fromTemplate(index,withFurniture=false){
 const t=TEMPLATES[index];if(!t)throw Error('방 구조를 선택하세요.');
 const doc=newDocument({w:t[2],d:t[3],name:t[0].replace(/^[\d.]+평\s*/, '')});
 doc.objects=t[5].map(([type,x,z,w,d,r])=>makeObject(type,{x,z,w,d,r:r||0,ownership:'fixed'}));
 // Do not force oversized suggested furniture into an obstructed room.
 if(withFurniture){
  for(const [type,ratio,preferredR] of [['bed',[.25,.22],0],['desk',[.73,.3],0],['chair',[.7,.5],0],['standby',[.47,.58],0]]){
   const obj=makeObject(type,{ownership:'unknown',placed:false});const candidates=[];
   for(const r of [preferredR,(preferredR+90)%360])for(let z=20;z<doc.room.d-20;z+=20)for(let x=20;x<doc.room.w-20;x+=20){
    const o={...obj,x,z,r};if(placementWarnings(o,doc.objects.filter(p=>p.placed),doc.room).length)continue;
    // Leave a short entry zone clear; this is not a full circulation guarantee.
    const door=doc.objects.find(p=>p.type==='door');if(door&&Math.hypot(x-door.x,z-door.z)<100)continue;
    candidates.push({x,z,r,score:Math.hypot(x-doc.room.w*ratio[0],z-doc.room.d*ratio[1])});
   }
   candidates.sort((a,b)=>a.score-b.score);if(candidates[0])Object.assign(obj,{...candidates[0],placed:true});doc.objects.push(obj);
  }
 }
